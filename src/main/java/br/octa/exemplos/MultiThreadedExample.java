...
/**
 * MultiThreadedExample is rather complex. It demonstrates how to use the
 * SessionReferenceProvider defined in the package com.sap.conn.jco.ext.
 * 
 * Before discussing situations requiring SessionReferenceProvider, we provide a
 * short description of how the JCo Runtime handles the stateful and stateless
 * calls by default. By default all RFC calls
 * (JCoFunction.execute(JCoDestination)) are stateless. That means the ABAP
 * context associated with the connection will be destroyed. Some RFC modules
 * save a particular state/data in the ABAP context's area. In order to keep a
 * JCo connection and use it for subsequent (stateful) calls, the
 * JCoConext.begin(JCoDestination) API can be used. In the case of multithreaded
 * applications some calls to a destination can be executed concurrently, so JCo
 * Runtime needs to associate a particular call or connection to an internal
 * session. By default JCo Runtime associates each thread with a sessionof its
 * own, so that most applications that execute all stateful requests en bloc or
 * at least in the same thread will run correctly.
 * 
 * Applications that wish to execute calls belonging to a stateful sequence by
 * employing different threads have to implement and register the
 * SessionReferenceProvider. The main goal of the implementation is to determine
 * to which session the calls executing in the current thread belong.
 * 
 * This example defines MultiStepJob having several execution steps. The test
 * starts a certain number of threads (see runJobs). Each thread is designed to
 * take a job, execute one step, and put the job back to the shared job list.
 * There are two jobs as an example: StatelessMultiStepExample and
 * StatefulMultiStepExample. Both invoke the same RFC modules, but
 * StatefulMultiStepExample uses JCoContext.begin and JCoContext.end to specify
 * the stateful calls.
 * 
 ...
 * To achieve that WorkerThread.run sets this session reference before executing
 * the next step and removes it after the step is finished.
 */
