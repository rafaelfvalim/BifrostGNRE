...
/**
 * Metadata Repository in the JCo 2.x offers chaining of repositories. The idea
 * behind was to enable automatically the look up in more than one SAP system
 * and/or in the custom repository/repositories.
 * 
 * This feature is not supported by JCo 3.0, because it was rarely used in the
 * past and because of troubles caused by incompatible metadata in different
 * systems and out-dated custom repositories. However the chain of repositories
 * can be easily implemented and effectively used in some specific scenarios.
 * 
 * The class RepositoryChain demonstrates how the repository chain can be
 * implemented and provides some tests in the main function. The implementation
 * can be changed for the application needs.
 * 
 * Note: the repository monitor was not implemented for the RepositoryChain in
 * this example. Note: the implementation does not check the cycles in the chain
 */
public class RepositoryChain implements JCoRepository {
	 ...
	 * @param repository
	 *            for the next chain
	 * @return removed repository if the next chain was already specified or
	 *         null
	 */
	 ...
	 * sets the next chain
	 * 
	 * @param repositoryChain
	 *            the next chain
	 * @return removed repository chain if the next chain was already specified
	 *         or null
	 */
	...
	/**
	 * returns a function interface, if available in one of the repositories in
	 * the chain or null
	 * 
	 * @param functionName
	 *            function name
	...
	/**
	 * returns a function template, if available in one of the repositories in
	 * the chain or null
	 * 
	 * @param functionName
	 *            function name
	...
	/**
	 * returns a record metadata (structure) description, if available in one of
	 * the repositories in the chain or null
	 * 
	 * @param recordName
	...
	/**
	 * returns a request metadata, if available in one of the repositories in
	 * the chain or null
	 * 
	 * @param functionName
	 *            function name
	...
	/**
	 * returns a structure description, if available in one of the repositories
	...
	/**
	 * returns true if all repositories in the chain contains the unicode
	 * metadata
	 */
	...
	public void save(Writer writer) throws IOException {
		((RepositoryChain) current).save(writer);
		if (next != null) {
			writer.write(",");
	...
	public void load(Reader reader) throws IOException {
		((RepositoryChain) current).load(reader);
		if (next != null) {
